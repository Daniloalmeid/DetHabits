// Função para esperar o carregamento das bibliotecas
function waitForSolanaLibraries() {
    return new Promise((resolve, reject) => {
      const maxAttempts = 10;
      let attempts = 0;
      const checkInterval = setInterval(() => {
        if (
          window.SolanaWalletAdapterBase &&
          window.SolanaWalletAdapterWallets &&
          window.solana &&
          window.solana.isPhantom
        ) {
          clearInterval(checkInterval);
          console.log("Bibliotecas Solana carregadas com sucesso.");
          resolve();
        } else if (attempts >= maxAttempts) {
          clearInterval(checkInterval);
          console.error("Falha ao carregar bibliotecas Solana após", maxAttempts, "tentativas.");
          reject(new Error("Solana libraries not loaded"));
        }
        attempts++;
      }, 500);
    });
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
      // Esperar carregamento das bibliotecas
      await waitForSolanaLibraries();
  ...
  async function connectWallet() {
    ...
    if (!checkPhantom()) {
      console.error("Phantom não encontrado.");
